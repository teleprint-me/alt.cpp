...
#define FIXED_VAL \
    (1 << FIXED_SIZE) // 2^16, for scaling between int and fixed-point
...
// Converts a floating-point number to fixed-point format (avoid on integer-only
// CPUs)
#define FLOAT_TO_FIXED(x) ((fixed16_t) ((x) * FIXED_VAL))

// Converts a fixed-point number to floating-point format (avoid on integer-only
// CPUs)
